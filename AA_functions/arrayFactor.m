function [AF, u, v, w] = arrayFactor(xPos, yPos, zPos, elementWeights, f, c, thetaScanAngles, phiScanAngles, thetaSteerAngle, phiSteerAngle, pertStDev)
...
if ~exist('pertStDev', 'var')
    pertStDev = ones(1,length(xPos));
elseif ~isvector(pertStDev)
    error('Pertubation must be a 1xP vector where P is number of elements')
end
% Wavenumber
k = 2*pi*f/c;
...
gain = repmat(reshape(elementWeights, 1, 1, P), M, N);
perturbation = repmat(reshape(pertStDev, 1, 1, P), M, N);
AF = sum(gain.*exp(1j*k*(uu + vv + ww)).*perturbation, 3);
% Normalising
