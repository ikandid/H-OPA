function out = RunGA_3_6(problem, params)
    ...
    for i = 1:nPop
        ...
        pop(i).var=nan;
        pop(i).var=repmat(pop(1).var, 1, 16);
        pop(i).var = Binary_conversion(xhi,xlo,nbits, nVar/nbits, pop(i).Position);
        
        %Evaluate Solution
        pop(i).Cost = CostFunction(pop(i).var(6:16),pop(i).var(1:5));
    ...
    for it = 1:MaxIt
        ...
        for l = 1:nC
            ...
            %Evaluate 
            popc(l).Cost = CostFunction(popc(l).var(6:16),popc(l).var(1:5));
